...
namespace PXengine.Core
{
    ...
    /// </summary>
    public class GameFactory: TTFactory
    {
        ...
        public Entity CreateSubtitle(string text, Color color)
        {
            ...
            e.C<DrawComp>().DrawColor = color;
            return e;
        }

        public Entity CreateSubtitle(SubtitleText stComp)
        {
            var e = AddDrawable(New());
            e.AddC(stComp);
            e.Refresh();
            return e;
        }
        ...
        {
            var e = CreateSprite(New(),bitmapFile);
